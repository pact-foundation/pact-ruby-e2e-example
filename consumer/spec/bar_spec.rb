...
describe "BarClient", :pact => true do
  it "can retrieve a thing"  do
    ...
    bar_response = Faraday.get(bar_service.mock_service_base_url + ENV['PACT_REQUEST_PATH'], nil, {'Accept' => 'application/json'})
  ...
  it "can retrieve a thing"  do
    bar_service.
      upon_receiving("a retrieve thing request that always passes").with({
      method: :get,
      path: '/anotherthing',
      headers: {'Accept' => 'application/json'}
    }).
    ...
    # This request would normally be performed some BarClient class,
    # but just use simple request for the purposes of this test
    bar_response = Faraday.get(bar_service.mock_service_base_url + '/anotherthing', nil, {'Accept' => 'application/json'})
